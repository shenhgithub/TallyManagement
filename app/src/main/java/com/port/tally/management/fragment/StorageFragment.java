...
public class StorageFragment extends BaseCodeListFragment<Storage, String> {
    ...
    @Override
    protected void onFillDataList(@Nullable List<Storage> dataList, List<Map<String, String>>
            mapList) {
        if (dataList == null) {
            // 数据加载失败或未完成
            return;
    ...
    protected String onActionTag() {
        return StaticValue.CodeListTag.STORAGE_LIST;
    }

    @Override
    protected EditText onFilterEditText(View rootView) {
        return (EditText) getActivity().findViewById(R.id.storage_edit_editText);
    }
